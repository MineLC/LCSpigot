...
public final class Title {
  ...
  public Title(BaseComponent title, BaseComponent subtitle, int fadeIn, int stay, int fadeOut) {
    this(new BaseComponent[1], 
        (subtitle == null) ? null : new BaseComponent[1], fadeIn, stay, fadeOut);
  }
  
  public Title(BaseComponent[] title, BaseComponent[] subtitle, int fadeIn, int stay, int fadeOut) {
    Preconditions.checkArgument((fadeIn >= 0), "Negative fadeIn: %s", new Object[] { Integer.valueOf(fadeIn) });
    Preconditions.checkArgument((stay >= 0), "Negative stay: %s", new Object[] { Integer.valueOf(stay) });
    Preconditions.checkArgument((fadeOut >= 0), "Negative fadeOut: %s", new Object[] { Integer.valueOf(fadeOut) });
  ...
  public Title(String title, String subtitle, int fadeIn, int stay, int fadeOut) {
    this(
        TextComponent.fromLegacyText((String)Preconditions.checkNotNull(title, "title")), (subtitle == null) ? null : 
        TextComponent.fromLegacyText(subtitle), fadeIn, stay, fadeOut);
  }
