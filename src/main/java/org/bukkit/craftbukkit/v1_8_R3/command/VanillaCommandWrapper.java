...
public final class VanillaCommandWrapper extends VanillaCommand {
    ...
    public final int dispatchVanillaCommand(CommandSender bSender, ICommandListener icommandlistener, String[] as) {
        // Copied from net.minecraft.server.v1_8_R3.CommandHandler
        ...
        try {
            ...
            if (icommandlistener.f() instanceof EntityMinecartCommandBlock) {
                Logger.warn(String.format("MinecartCommandBlock at (%d,%d,%d) failed to handle command", icommandlistener.getChunkCoordinates().getX(), icommandlistener.getChunkCoordinates().getY(), icommandlistener.getChunkCoordinates().getZ()), throwable);
                ...
                Logger.warn(String.format("CommandBlock at (%d,%d,%d) failed to handle command", listener.getChunkCoordinates().getX(), listener.getChunkCoordinates().getY(), listener.getChunkCoordinates().getZ()), throwable);
            } else {
                Logger.warn(String.format("Unknown CommandBlock failed to handle command"), throwable);
            }
        } finally {
