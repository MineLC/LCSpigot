        ...
        for (int i = 0; i < commands.size(); i++) {
            try {
                ...
                if (sender.f() instanceof EntityMinecartCommandBlock) {
                    Logger.warn(String.format("MinecartCommandBlock at (%d,%d,%d) failed to handle command", sender.getChunkCoordinates().getX(), sender.getChunkCoordinates().getY(), sender.getChunkCoordinates().getZ()), exception);
                    ...
                    Logger.warn(String.format("CommandBlock at (%d,%d,%d) failed to handle command", listener.getChunkCoordinates().getX(), listener.getChunkCoordinates().getY(), listener.getChunkCoordinates().getZ()), exception);
                } else {
                    Logger.warn(String.format("Unknown CommandBlock failed to handle command"), exception);
                }
            }
        }
