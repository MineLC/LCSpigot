...
public class NameReferencingFileConverter {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    ...
                    public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
                        Logger.warn("Could not lookup oplist entry for " + gameprofile.getName(), exception);
        ...
        if (NameReferencingFileConverter.d.exists() && NameReferencingFileConverter.d.isFile()) {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    ...
                    public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
                        Logger.warn("Could not lookup user whitelist entry for " + gameprofile.getName(), exception);
                        if (!(exception instanceof ProfileNotFoundException)) {
                            throw new NameReferencingFileConverter.FileConversionException("Could not request user " + gameprofile.getName() + " from backend systems", exception, null);
                        }
                    }
        ...
        if (file.exists() && file.isDirectory()) {
            ...
            try {
                ...
                ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
                    ...
                    public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
                        Logger.warn("Could not lookup user uuid for " + gameprofile.getName(), exception);
                        if (exception instanceof ProfileNotFoundException) {
    ...
    private static boolean b(PropertyManager propertymanager) {
        ...
        if (!flag2 && !flag3) {
            ...
            if (flag2) {
                Logger.warn("* " + NameReferencingFileConverter.c.getName());
            }

            if (flag3) {
                Logger.warn("* " + NameReferencingFileConverter.d.getName());
            }
