...
public abstract class World implements IBlockAccess {
    ...
    public void tickEntities() {
        ...
        for (tileLimiter.initTick();
            ...
            // Spigot start
            if (tileentity == null) {
                Logger.error("Spigot has detected a null entity and has removed it, preventing a crash");
                tilesThisCycle--;
        ...
        if (this.L == 0 && !this.isClientSide) {
            ...
            if (block.getMaterial() == Material.AIR && this.k(blockposition) <= random.nextInt(8) && this.b(EnumSkyBlock.SKY, blockposition) <= 0) {
                ...
                if (entityhuman != null && entityhuman.e((double) l + 0.5D, (double) j1 + 0.5D, (double) i1 + 0.5D) > 4.0D) {
                    this.makeSound((double) l + 0.5D, (double) j1 + 0.5D, (double) i1 + 0.5D, "ambient.cave.cave", 0.7F, 0.8F);
    ...
    public List<Entity> a(Entity entity, AxisAlignedBB axisalignedbb, Predicate<? super Entity> predicate) {
        ...
        if (j - i > 10 || l - k > 10) {
            Logger.warn( "Filtered out large getEntities call {0},{1} {2},{3}", new Object[]{i, j, k, j});
            return arraylist;
        }
        // CraftBukkit end
